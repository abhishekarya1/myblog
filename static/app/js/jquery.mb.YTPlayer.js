$(document).ready(function($) {
    ...
    if( /Android|webOS|Phone|iPad|iPod|BlackBerry|Windows Phone/i.test(navigator.userAgent) ) { onMobile = true; }
    if( ( onMobile === false ) ) {
...
( function( jQuery, ytp ) {
	...
	jQuery.mbYTPlayer = {
		...
		addMask: function( mask ) {
			...
			var tempImg = jQuery( "<img-app/>" ).attr( "src", mask ).on( "load", function() {
				...
				if( typeof YTPlayer.opt.mask == "string" ) {
					...
					for( var time in YTPlayer.opt.mask ) {
						if( YTPlayer.opt.mask[ time ] )
							var img = jQuery( "<img-app/>" ).attr( "src", YTPlayer.opt.mask[ time ] );
					}
